...
> 4. load类指令与store类指令略有不同，由于这个存储器只配置了片选使能和字节写使能，所以读取的时候一律是先读回CPU（此时不区分是哪个load指令），再进行更细分的操作。那么就需要把细分的指令操作随流水线传到MEM段，使MEM段能知道应该根据哪种方式处理数据。
...
> 7. 修改方法有两种:
>> 第一种直接把ID到EX的线宽改掉，在EX段对四位的选择信号进行赋值。EX到MEM由于MEM段中load指令分析时本身就需要四位的字节选位，不需要进行修改。（改的时候希望各位不会漏改和改错）
>> 第二种不修改线宽，直接借用已有的四位通道，自己对控制信号进行一些编码和解码，达到和第一种一样的效果。
